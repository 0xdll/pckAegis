...
|--------|------|------------|-------------|
| `-file` | string | *requis* | Chemin vers `requirements.txt` ou `package.json` |
| `-eco` | string | `PyPI` | Écosystème ciblé (`PyPI` ou `npm`) |
| `-out` | string | *(vide)* | Chemin de sortie pour rap -out security-report.json
...
Alternative Docker dans un pipeline GitLab CI/CD :
```yaml
osv_scan:
  image: docker:stable
  services:
  ...
  script:
    - docker build -t pckaegis .
    - docker run --rm -v $CI_PROJECT_DIR:/workspace pckaegis -file /workspace/package.json -eco npm -out report.json
  artifacts:
    paths:
      - report.json
    when: always
      - uses: actions/setup-go@v5
        with:
          go-version: '1.25.5'
      - name: Build scanner
        run: go build -o pckaegis ./cmd/scan
      - name: Scan dependencies (PyPI)
        run: ./pckaegis -file requirements.txt -eco PyPI
```
...
osv_scan:
  ...
  script:
    - docker build -t pckaegis .
    - docker run --rm -v $CI_PROJECT_DIR:/workspace pckaegis -file /workspace/package.json -eco npm
```
è
## Notes
